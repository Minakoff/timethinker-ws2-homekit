 ...
 * a simple Timethinker WS2 using HomeKit.
 ...
 *
 * In order to flash the Timethinker WS2 you will have to
 ...
 * to your computer. The Timethinker WS2 is now in flash mode and
 * you can flash the custom firmware.
 *
 * WARNING: Do not connect the Timethinker WS2 to AC while it's
 * connected to the FTDI adapter! This may fry your
 * computer and Timethinker WS2.
 *
 */
...
// The GPIO pin that is connected to last pin of the programming strip of the Timethinker WS2.
const int pin_gpio = 15;
// The GPIO pin that is connected to the relay on the Timethinker WS2.
const int relay_gpio = 14;
// The GPIO pin that is connected to the LED on the Timethinker WS2.
const int led_gpio = 13;
// The GPIO pin that is connected to the button on the Timethinker WS2.
...
void reset_configuration() {
    printf("Resetting WS2 configuration\n");
...
void switch_identify_task(void *_args) {
    // We identify the Timethinker WS2 by Flashing it's LED.
    for (int i=0; i<3; i++) {
        led_blink(2);
    }
...
homekit_characteristic_t name = HOMEKIT_CHARACTERISTIC_(NAME, "Timethinker WS2");

homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_switch, .services=(homekit_service_t*[]){
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]){
            &name,
            HOMEKIT_CHARACTERISTIC(MANUFACTURER, "Timethinker"),
            HOMEKIT_CHARACTERISTIC(SERIAL_NUMBER, "037A2BABF19D"),
            HOMEKIT_CHARACTERISTIC(MODEL, "WS2"),
            HOMEKIT_CHARACTERISTIC(FIRMWARE_REVISION, "1.0.0"),
            HOMEKIT_CHARACTERISTIC(IDENTIFY, switch_identify),
            NULL
        }),
        HOMEKIT_SERVICE(SWITCH, .primary=true, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "Timethinker WS2"),
            &switch_on,
            NULL
        }),
        NULL
...
void create_accessory_name() {
    ...
    int name_len = snprintf(NULL, 0, "Timethinker-%02X%02X%02X",
                            macaddr[3], macaddr[4], macaddr[5]);
    char *name_value = malloc(name_len+1);
    snprintf(name_value, name_len+1, "Timethinker-%02X%02X%02X",
             macaddr[3], macaddr[4], macaddr[5]);
...
void user_init(void) {
    ...
    wifi_config_init("TimethinkerWS2", NULL, on_wifi_ready);
    gpio_init();
